...
import net.mcreator.slightlyvanillaadditions.procedures.ShulkSoldierOnInitialEntitySpawnProcedure;
...
public class ShulkSoldierEntity extends Monster {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MONSTER)
					.add(new MobSpawnSettings.SpawnerData(SlightlyVanillaAdditionsModEntities.SHULK_SOLDIER.get(), 4, 1, 2));
	}

	public ShulkSoldierEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(SlightlyVanillaAdditionsModEntities.SHULK_SOLDIER.get(), world);
	}

	public ShulkSoldierEntity(EntityType<ShulkSoldierEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(SlightlyVanillaAdditionsModItems.DRAGON_SWORD.get()));
	}
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(Items.SHULKER_SHELL));
	}
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason,
		...
		ShulkSoldierOnInitialEntitySpawnProcedure.execute(this);
		return retval;
	}

	public static void init() {
		SpawnPlacements.register(SlightlyVanillaAdditionsModEntities.SHULK_SOLDIER.get(), SpawnPlacements.Type.ON_GROUND,
