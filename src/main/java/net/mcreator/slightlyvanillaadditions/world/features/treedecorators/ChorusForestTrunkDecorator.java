...
public class ChorusForestTrunkDecorator extends TrunkVineDecorator {
	public static final ChorusForestTrunkDecorator INSTANCE = new ChorusForestTrunkDecorator();
	public static com.mojang.serialization.Codec<ChorusForestTrunkDecorator> codec;
	public static TreeDecoratorType<?> tdt;
	static {
		codec = com.mojang.serialization.Codec.unit(() -> INSTANCE);
		tdt = new TreeDecoratorType<>(codec);
		tdt.setRegistryName("chorus_forest_tree_trunk_decorator");
		...
		listBlockPos.forEach(blockpos -> {
			if (random.nextInt(3) > 0) {
				...
				if (Feature.isAir(levelReader, bp)) {
					biConsumer.accept(blockpos, SlightlyVanillaAdditionsModBlocks.CHORUS_LEAVES.get().defaultBlockState());
				}
			}
		});
	}
