...
public class BurnedDesertBiome extends SlightlyVanillaAdditionsModElements.ModElement {
	@ObjectHolder("slightly_vanilla_additions:burned_desert")
	public static final CustomBiome biome = null;
	public BurnedDesertBiome(SlightlyVanillaAdditionsModElements instance) {
		super(instance, 113);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(BurnedTreeLogBlock.block.getDefaultState()),
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.SAND.getDefaultState().getBlock() || ground == Blocks.SAND.getDefaultState().getBlock())
							&& (ground2 == Blocks.SAND.getDefaultState().getBlock() || ground2 == Blocks.SAND.getDefaultState().getBlock())))
						return false;
					...
					if (position.getY() < world.getHeight() - height - 1) {
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, BurnedTreeLogBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BurnedTreeLogBlock.block.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == Blocks.SAND.getDefaultState().getBlock()
