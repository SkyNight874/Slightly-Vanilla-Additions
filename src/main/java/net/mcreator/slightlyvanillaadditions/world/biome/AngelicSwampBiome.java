...
public class AngelicSwampBiome extends SlightlyVanillaAdditionsModElements.ModElement {
	@ObjectHolder("slightly_vanilla_additions:angelic_swamp")
	public static final CustomBiome biome = null;
	public AngelicSwampBiome(SlightlyVanillaAdditionsModElements instance) {
		super(instance, 126);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.SWAMP).waterColor(4159204).waterFogColor(329011).parent("slightly_vanilla_additions:mossy_jungle")
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(EndMyceliumBlock.block.getDefaultState(),
							Blocks.END_STONE.getDefaultState(), Blocks.END_STONE.getDefaultState())));
			setRegistryName("angelic_swamp");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
					new CustomTreeFeature()
							.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(Blocks.AIR.getDefaultState()),
									new SimpleBlockStateProvider(Blocks.AIR.getDefaultState()))).baseHeight(10)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == EndMyceliumBlock.block.getDefaultState().getBlock() || ground == Blocks.END_STONE.getDefaultState().getBlock())
							&& (ground2 == EndMyceliumBlock.block.getDefaultState().getBlock()
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.AIR.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == EndMyceliumBlock.block.getDefaultState().getBlock()
