...
public class MossyJungleBiome extends SlightlyVanillaAdditionsModElements.ModElement {
	@ObjectHolder("slightly_vanilla_additions:mossy_jungle")
	public static final CustomBiome biome = null;
	public MossyJungleBiome(SlightlyVanillaAdditionsModElements instance) {
		super(instance, 122);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.NONE).waterColor(-13369345).waterFogColor(-6684673).parent("slightly_vanilla_additions:chorus_forest")
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(EndMossBlock.block.getDefaultState(),
							Blocks.END_STONE.getDefaultState(), Blocks.END_STONE.getDefaultState())));
			setRegistryName("mossy_jungle");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
					new CustomTreeFeature()
							.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(Blocks.JUNGLE_LOG.getDefaultState()),
									new SimpleBlockStateProvider(MossBlockBlock.block.getDefaultState()))).baseHeight(10)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == EndMossBlock.block.getDefaultState().getBlock() || ground == Blocks.END_STONE.getDefaultState().getBlock())
							&& (ground2 == EndMossBlock.block.getDefaultState().getBlock()
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == MossBlockBlock.block.getDefaultState().getBlock()
												|| state.getBlock() == MossBlockBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, MossBlockBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getMaterial().blocksMovement() || state.isIn(BlockTags.LEAVES)
								...
								if (genh > 0) {
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(-1, genh, 0)))
										setTreeBlockState(changedBlocks, world, position.add(-1, genh, 0), MossBlockBlock.block.getDefaultState(),
												bbox);
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(1, genh, 0)))
										setTreeBlockState(changedBlocks, world, position.add(1, genh, 0), MossBlockBlock.block.getDefaultState(),
												bbox);
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(0, genh, -1)))
										setTreeBlockState(changedBlocks, world, position.add(0, genh, -1), MossBlockBlock.block.getDefaultState(),
												bbox);
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(0, genh, 1)))
										setTreeBlockState(changedBlocks, world, position.add(0, genh, 1), MossBlockBlock.block.getDefaultState(),
												bbox);
						...
						for (int genh = position.getY() - 3 + height; genh <= position.getY() + height; genh++) {
							...
							for (int genx = position.getX() - k4; genx <= position.getX() + k4; genx++) {
								for (int genz = position.getZ() - k4; genz <= position.getZ() + k4; genz++) {
									...
									if (state.isIn(BlockTags.LEAVES) || state.getBlock() == MossBlockBlock.block.getDefaultState().getBlock()) {
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world, position.add(dir.getXOffset(), height - 5 + hlevel, dir.getZOffset()),
												AngelicSlimeBlockBlock.block.getDefaultState(), bbox);
									}
								}
							}
						}
		...
		private void addVines(IWorld world, BlockPos pos, Set<BlockPos> changedBlocks, MutableBoundingBox bbox) {
			setTreeBlockState(changedBlocks, world, pos, MossBlockBlock.block.getDefaultState(), bbox);
			int i = 5;
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.JUNGLE_LOG.getDefaultState().getBlock()
					|| blockType == MossBlockBlock.block.getDefaultState().getBlock() || blockType == EndMossBlock.block.getDefaultState().getBlock()
