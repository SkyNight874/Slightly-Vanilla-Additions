...
import net.mcreator.slightlyvanillaadditions.world.biome.AlteratedForestNetherBiome;
...
public class SlightlyVanillaAdditionsModBiomes {
	...
	public static final RegistryObject<Biome> ALTERATED_FOREST_NETHER = REGISTRY.register("alterated_forest_nether",
			() -> AlteratedForestNetherBiome.createBiome());
	...
	public static void init(FMLCommonSetupEvent event) {
		event.enqueueWork(() -> {
			...
			AlteratedForestNetherBiome.init();
		});
	}
		...
		public static void onServerAboutToStart(ServerAboutToStartEvent event) {
			...
			for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : worldGenSettings.dimensions().entrySet()) {
				...
				if (dimensionType == dimensionTypeRegistry.getOrThrow(DimensionType.NETHER_LOCATION)) {
					...
					if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
						List<Pair<Climate.ParameterPoint, Holder<Biome>>> parameters = new ArrayList<>(noiseSource.parameters.values());
						parameters.add(new Pair<>(AlteratedForestNetherBiome.PARAMETER_POINT,
								biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, ALTERATED_FOREST_NETHER.getId()))));
						MultiNoiseBiomeSource moddedNoiseSource = new MultiNoiseBiomeSource(new Climate.ParameterList<>(parameters),
								noiseSource.preset);
						chunkGenerator.biomeSource = moddedNoiseSource;
						chunkGenerator.runtimeBiomeSource = moddedNoiseSource;
					...
					if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
						...
						if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
							List<SurfaceRules.RuleSource> surfaceRules = new ArrayList<>(sequenceRuleSource.sequence());
							surfaceRules.add(2,
									anySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, ALTERATED_FOREST_NETHER.getId()),
											SlightlyVanillaAdditionsModBlocks.ALTERATED_NETHERRACK.get().defaultBlockState(),
