...
public class BackPackItemInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	...
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == SlightlyVanillaAdditionsModItems.BACK_PACK_ITEM.get()) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(18) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != SlightlyVanillaAdditionsModItems.BACK_PACK_ITEM.get();
			}
